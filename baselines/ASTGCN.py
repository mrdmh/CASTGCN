...
class ASTGCN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, nb_block=4, in_channels=40, K=5,
                 nb_chev_filter=64, nb_time_filter=64, time_strides=1,
                 num_for_predict=1, len_input=10, num_of_vertices=36):
        """
        :param nb_block:
        :param in_channels:
        :param K:
        ...
        self.BlockList = nn.ModuleList([ASTGCN_block(DEVICE, in_channels, K,
                                                     nb_chev_filter, nb_time_filter,
                                                     time_strides, cheb_polynomials,
                                                     num_of_vertices, len_input)])

        self.BlockList.extend([ASTGCN_block(DEVICE, nb_time_filter, K,
        ...
        self.time_conv = nn.Sequential(
            ...
            nn.Conv1d(in_channels=nb_time_filter,
                      out_channels=nb_time_filter,
                      kernel_size=int(len_input/time_strides) - 1,
                      stride=1),
            nn.BatchNorm1d(nb_time_filter),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        b, n, f, t = x.shape
        # x: [B, N, F, T] -> [B*N, F, T] -> [B*N, hidden, 1]
        output = self.time_conv(x.reshape(-1, f, t))
        output = output.squeeze().reshape(b, n, -1)
        return output
